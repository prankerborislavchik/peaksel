...
export async function ownerMiddleware(request: NextRequest) {
    // Короче, на ЛЮБОЙ запрос на владельца, будет проверяться:
    ...
    if (!tokenService.hasRole(token, 'OWNER')) throw ErrorApi
        .forbidden('Данная функция доступна только владельцу')
